...
:::danger
The Raspi 4 is no longer recommended for mainnet pools. It is possible to use it as a mainnet node for submitting tx's and learning. Though I would recommend
learning on testnet. 
:::

Cardano has two testnets. Preview and Preprod. On Preview epochs are one day long, while on Preprod they are 5 days like on mainnet. You can find more details
and the operational files on [https://book.world.dev.cardano.org/environments.html](https://book.world.dev.cardano.org/environments.html).

:::danger

There is a 500 ₳ Registration deposit and another 5 ₳ in registration costs to start a pool on mainnet. First time users are strongly recommended to use testnet. 
You can get tada (test ada) from the testnet faucets.
...
You can get a good snapshot for mainnet here.
